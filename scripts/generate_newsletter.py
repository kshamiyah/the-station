...
def generate_archive_section(data):
    """Previous editions archive section with link to archive page."""
    if not data or not data.get('archive_link'):
        return ""

    archive_link = (data.get('archive_link') or '').strip()
    if not archive_link:
        return ""
    ...
    <div style="font-family: {STYLE['font_stack']}; font-size: 18px; font-weight: 600; color: {STYLE['text_primary']}; margin-bottom: 12px; letter-spacing: -0.3px;">
        Browse the Archive
    </div>

    <div style="font-family: {STYLE['font_stack']}; font-size: 14px; color: {STYLE['text_primary']}; line-height: 1.6; margin-bottom: 16px;">
        Missed a previous edition? View all past issues and stay caught up with our archive.
    </div>

    <a href="{escape_html(archive_link)}" style="display: inline-block; color: {STYLE['accent_blue']}; font-family: {STYLE['font_stack']}; font-size: 14px; font-weight: 500; text-decoration: none; padding: 12px 0; border-bottom: 2px solid {STYLE['accent_blue']};">
        View all editions ↗
    </a>
    """
    return generate_card_wrapper(content)
